        ...
        <div className="flex flex-wrap items-center gap-4 rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-4 shadow-sm">
          ...
          <div className="flex gap-1 bg-slate-100 dark:bg-slate-800 p-1 rounded-lg ml-auto">
             {[{ label: "All", v: null }, { label: "24h", v: "24h" }].map(t => (
                <Link key={t.label} href={getFilterUrl("timeframe", t.v)} className={`rounded-md px-4 py-1.5 text-xs font-bold transition-all ${(timeframe === (t.v || "")) ? "bg-white dark:bg-slate-700 text-slate-900 dark:text-white shadow-sm" : "text-slate-500 dark:text-slate-400 hover:text-slate-700"}`}>{t.label}</Link>
             ))}
          </div>
        </div>
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
              {jobs.map(job => (
                <ClickableRow key={job.id} url={job.url} className={`${job.status === "OFFER" ? "bg-emerald-50/40 dark:bg-emerald-900/10" : job.status === "INTERVIEWING" ? "bg-blue-50/40 dark:bg-blue-900/10" : job.status === "REJECTED" ? "opacity-50 grayscale" : "hover:bg-slate-50 dark:hover:bg-slate-800"}`}>
                  ...
                  <td className="px-6 py-5 align-top"><div className="text-slate-600 dark:text-slate-400 font-mono text-[11px] bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 w-fit px-2 py-1 rounded">{formatAppliedAt(job.appliedAt)}</div></td>
